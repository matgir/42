...
export function initialize() {
    ...
    // FLOOR 1
    const floorShape = new CANNON.Box(new CANNON.Vec3(4, paddle_level / 2, 0.0025)); // body très fin en profondeur
    const floorBody = new CANNON.Body({
        mass: 0,
        material: defaultMaterial,
        position: new CANNON.Vec3(0, 0, ecart -0.25)
    });
    floorBody.addShape(
        floorShape,
        new CANNON.Vec3(0, 0, 0.325) // décalage vers la face avant du mesh (la moitié de 0.7 - 0.05)
    );
    world.addBody(floorBody);
    ...
    // FLOOR 1
    const floorShape2 = new CANNON.Box(new CANNON.Vec3(4, paddle_level / 2, 0.0025)); // body très fin en profondeur
    const floorBody2 = new CANNON.Body({
        mass: 0,
        material: defaultMaterial,
        position: new CANNON.Vec3(0, 0, -ecart + 0.25)
    ...
    const countdown3Element = document.querySelector('.countdown3');
    if (countdown3Element) {
        countdown3Element.style.visibility = 'hidden'; // ou display: 'none' si tu préfères
    }
...
let test = false;
    ...
    function handleVictory() {
        ...
        body.velocity.set(22, 0, 0);
          ...
          countdownElement.style.transform = 'translateY(-30px)';
            ...
            countdownNumber.innerHTML = "VICTOIRE";
                ...
                const currentMatch0 = sessionStorage.getItem('currentMatch');
                console.log(currentMatch0)
        ...
        // if( gameState2 === 'victory' || fin_tournois == true )
        // {
            console.log(fin_tournois)
             setTimeout(() => {
                ...
                if (gameOverOverlay && winnerMessage && quitButton) {
                    ...
                    if (isTournamentPage) {
                        quitButton.textContent = "GET MY GIFT";
                        // quitButton.textContent = "GET MY GIFT";
                    ...
                    newQuitButton.addEventListener('click', function() {
                        ...
                        resetGame();
                            ...
                            window.location.href = "/2_pink_cubes/";
                    ...
                    console.error('Game over overlay elements not found in the DOM prout');
        ...
        // }
            ...
            const isTournamentPage = document.querySelector('.bracket') && document.querySelector('.bracket').children.length > 1;  
            if(fin_tournois == false && isTournamentPage )
               {
                console.log(fin_tournois)
                   ...
                   if (gameOverOverlay && winnerMessage && quitButton) {
                    ...
                    //    avatarOverlay.style.display = 'block';
                       ...
                       const isTournamentPage = document.querySelector('.bracket') && document.querySelector('.bracket').children.length > 1;
                       if (isTournamentPage) {
                           quitButton.textContent = "AGAIN";
                        quitButton.style.background = "none"

                            //  quitButton.style.color = "none"
                            //  quitButton.style.background = "none"
...
//////////////////////////////////////////////////
    ...
    let sphereColorIndex = 0; 
    let sphereInterpolationFactor = 0;
    ...
    function moveSmokeAvoidObject() 
    {
        ...
        const attractionStrength = 0.027; 
        const maxDistance = 13.5; 
        const airResistance = 0.99; 
        const dragCoefficient = 0.6; 
        const spread = 10; 
        const reboundFactor = 0.9; 
        const dispersionStrength = 0.0032; 
...
//////////////////////////////////////////////////////////////////
    ...
    function createVictoryCube() {
        ...
        const textures = [
            textureLoader.load('/static/globos/globo_de_8.png'), // Face droite (+X)
        ...
        victoryCube.rotation.set(0, Math.PI / 4, Math.PI / 3,);
