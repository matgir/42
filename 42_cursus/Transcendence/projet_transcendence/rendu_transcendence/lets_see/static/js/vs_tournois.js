...
// Using the global CANNON object that's loaded from the script tag instead
...
scene.add(directionalLight)
    ...
    const camera = new THREE.PerspectiveCamera(70, sizes.width / sizes.height, 0.1, 125)
    ...
    function transitionCamera(duration = 7000) {
...
// Gestion des clics sur "DUEL MODE"
document.querySelectorAll('.start_game').forEach(bouton => {
...
// Gestion des clics sur "TOURNAMENT"
document.querySelectorAll('.join_tournament').forEach(bouton => {
    bouton.addEventListener('click', (event) => {
        event.preventDefault();
        handleNavigationFadeOut(bouton, bouton.dataset.url);
        dark = 1;
...
return function cleanup (){
    console.log("clear game_home.js");
    clearInterval(sphereInterval);
    
    // Only remove our specific canvas if it exists
