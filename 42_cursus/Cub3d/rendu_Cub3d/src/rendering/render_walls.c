...
void	compose_wall_strip(t_vars *var, t_render *render, int i, t_data *text)
{
	...
	{
		render->distFromTop = y + (render->wallStripHeight / 2) - (WINDOW_HEIGHT / 2);
		render->offset_y = render->distFromTop * ((float)TEXT_SIZE / render->wallStripHeight);
		render->pxl = *(int *)(text->addr + (render->offset_y * text->line_length + render->offset_x * (text->bits_per_pixel / 8)));
		x = i * render->wallStripWidth;
		while (x < limit_x)
		{
...
void	render_walls(t_vars *var)
{
	...
	while (i < NUM_RAYS)
	{
		...
		render.wallStripHeight = (TILE_SIZE / ray.distance) * render.dist_proj_plane;
		render.wallStripWidth = WALL_STRIP_WIDTH;
		render.wallTopPxl = (WINDOW_HEIGHT / 2) - (render.wallStripHeight / 2);
		if (render.wallTopPxl < 0)
			render.wallTopPxl = 0;
		render.wallBottomPxl = (WINDOW_HEIGHT / 2) + (render.wallStripHeight / 2);
