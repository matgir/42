...
// distance of the player to the projection plan
var distanceProjPlane = (WINDOW_WIDTH / 2) / Math.tan(FOV_ANGLE / 2);

var	wallStripeHeight = (TILE_SIZE / ray.distance) * distanceProjPlane;

// for assigning wall textures NO EA SO WE
if (ray.isFacingDown && !ray.wasHitVertical)
{
	color = "<texture for the south>"
}
else if ((ray.isFacingDown || ray.isFacingUp) && ray.wasHitVertical && ray.isRayFacingLeft)
{
	color = "<texture for the west>"
}
else if ((ray.isFacingDown || ray.isFacingUp) && ray.wasHitVertical && ray.isRayFacingRight)
{
	color = "<texture for the east>"
}
else
{
	color = "<texture for north>"
}